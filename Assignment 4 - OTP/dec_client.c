...
int main(int argc, const char *argv[]) 
{
    ...
    const char *ciphertext_filename = argv[1];
    const char *key_filename = argv[2];
    int port_number = atoi(argv[3]);

    if (access(ciphertext_filename, F_OK) == -1) 
    {
    ...
    {
        handle_error("Client: ERROR connecting");
    }
    ...
    char *ciphertext_content, *key_content;
    long ciphertext_size, key_size;
    get_file_data(ciphertext_filename, &ciphertext_content, &ciphertext_size);
    ...
    //send ciphertext size and content
    char size_buffer[20];
    sprintf(size_buffer, "%ld", ciphertext_size);
    send(comm_socket, size_buffer, strlen(size_buffer), 0);
    recv(comm_socket, size_buffer, 20, 0);

    send(comm_socket, ciphertext_content, ciphertext_size, 0);
    recv(comm_socket, size_buffer, 20, 0);
    ...
    while (bytes_received < ciphertext_size) 
    {
