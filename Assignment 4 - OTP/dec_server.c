...
char *decrypt_data(char *message, char *key) 
{
    ...
    char *decrypted_text = malloc(sizeof(char) * (msg_len + 1));
    ...
    for (int i = 0; i < msg_len; i++) 
    {
        ...
        int decrypted_val = (msg_val - key_val + 27) % 27;
        decrypted_text[i] = integer_to_char(decrypted_val);
    }
...
int main(int argc, const char *argv[]) 
{
    ...
    while (1) 
    {
        ...
        if (child_pid == 0) 
        {
            ...
            char *decrypted_text = decrypt_data(ciphertext, key);
            send(connected_socket, decrypted_text, strlen(decrypted_text), 0);
